...
def _metric(solution_df, submission_df, mode="top_level", admin=False):
    ...
    if "time" in submission_df.columns:
        ...
        split = "public"
        evaluation[f"{split}_score"]["total_time"] = float(
            solution_df.query(f"split=='{split}'")["submission_time"].sum()
        )
    ...
    if "score" in submission_df.columns:
        ...
        split = "public"
        evaluation[f"{split}_score"]["fail_rate"] = float(
            solution_df.query(f"split=='{split}'")["submission_score"].isna().mean()
        )
